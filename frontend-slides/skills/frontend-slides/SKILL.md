...
description: Create stunning, animation-rich HTML presentations from scratch or by converting PowerPoint files. Use when the user wants to build a presentation, convert a PPT/PPTX to web, or create slides for a talk/pitch. Helps non-designers discover their aesthetic through visual exploration rather than abstract choices. Enforces IBM Developer brand guidelines (fonts, colors, layouts) when generating IBM-branded presentations.
---
...
6. **IBM Brand Compliance (NON-NEGOTIABLE when applicable)** — When generating IBM-branded presentations, ALL design decisions (fonts, colors, spacing, layout) MUST follow [IBM_BRAND_GUIDELINES.md](IBM_BRAND_GUIDELINES.md). Generic style presets are overridden entirely.

---

## IBM Brand Mode (MANDATORY OVERRIDE)

**Before any style or design decisions, check if this is an IBM-branded presentation.**

### Trigger Conditions — read [IBM_BRAND_GUIDELINES.md](IBM_BRAND_GUIDELINES.md) and apply it fully if ANY of these are true:

- User mentions **IBM**, **IBM Developer**, **IBM Cloud**, **IBM Watson**, or any IBM product/brand
- The file being converted is `IBM_Developer_Master_Presentation_2021_R01_Plex.potx` or any IBM-branded `.pptx`/`.potx`
- User says "follow IBM brand guidelines" or "IBM format"

### In IBM Brand Mode:

1. **Read `IBM_BRAND_GUIDELINES.md` immediately** — it is the single source of truth for all design decisions
2. **Skip the Phase 2 style selection** — IBM brand is mandatory, not a choice. Inform the user the IBM brand system will be applied
3. **Fonts**: IBM Plex Sans (headings bold 700, body regular 400, light 300) + IBM Plex Mono (code). Load from Google Fonts — NEVER use any other font family
4. **Colors**: Use the IBM palette exactly as defined in the guidelines. Default theme = White (`#FFFFFF` bg, `#000000` text, `#1261FD` accent). Cover/divider slides use Blue theme (`#1261FD` bg, white text)
5. **Layout**: Follow the grid (44px side padding, 42px top, 54px footer), two-column content pattern, and layout catalogue from the guidelines
6. **No cover page images** — IBM explicitly prohibits photos/images on cover slides
7. **Footer bar** on every content slide: `Group Name / DOC ID / Month YYYY / © YYYY IBM Corporation`
8. **Animation**: Simple, professional — fade-up reveals at 300ms, `ease-out`. No particles, no gradients, no decorative motion
9. **Always use the CSS variables** defined in the guidelines' Section 10

### IBM Theme Variants

Generate all slides using the **White theme** by default. Apply `class="theme-blue"` to cover slides, divider slides, and any emphasis slide where blue background is appropriate.
...
- [IBM_BRAND_GUIDELINES.md](IBM_BRAND_GUIDELINES.md) — **Read this first** if generating an IBM-branded presentation
...
| -------------------------------------------------- | -------------------------------------------------------------------- | ------------------------- |
| [IBM_BRAND_GUIDELINES.md](IBM_BRAND_GUIDELINES.md) | **IBM brand system**: fonts, colors, layout, spacing, slide types    | **Immediately** for IBM decks (before Phase 2) |
| [STYLE_PRESETS.md](STYLE_PRESETS.md)               | 12 curated visual presets with colors, fonts, and signature elements | Phase 2 (style selection, non-IBM only) |
| [bold-template-pack/selection-index.json](bold-template-pack/selection-index.json) | Compact bold template metadata for candidate selection | Phase 2 (style selection, non-IBM only) |
| [bold-template-pack/templates/*/preview.md](bold-template-pack/templates/) | Lightweight style cards for shortlisted bold title previews | Phase 2 after shortlisting (non-IBM only) |
| [bold-template-pack/templates/*/design.md](bold-template-pack/templates/) | Detailed design-system docs for the selected bold template only | Phase 3 after user selection (non-IBM only) |
